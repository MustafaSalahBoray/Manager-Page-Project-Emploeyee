			...
			<form action="" id="manage_evaluation" method="POST">
				...
				<div class="row">
					<div class="col-md-6 border-right">
							...
							<label for="" class="control-label">Task</label>
                                ...
                                <input type="text" class="form-control" name="task_id" value="'.$key['Task'].'"</div>
								';
                              ?>
							...
							<label for="" class="control-label">Manager</label>
                                ...
                                <input type="text" class="form-control" name="manager_id" value="<?php echo $key['Name'];?>">
				...
				<div class="col-lg-12 text-right justify-content-center d-flex">
					<button class="btn btn-primary mr-2" type="submit" name="submit" value="<?php echo $key['id'];}}?>">Save</button>
									 <?php
                     $selectAdmin=$db->prepare("SELECT * FROM admin");
                     $selectAdmin->execute();
				 ...
				 ?>
				<input type="hidden" name="name" value="<?php echo $key['name']?>"><?php }?>
					<button class="btn btn-secondary" type="button" onclick="location.href = 'index.php?page=evaluation'">Cancel</button>
				</div>
				<?php
				if (isset($_POST['submit'])) {
				   $INSERT=$db->prepare("UPDATE  evaluation  SET Task=:Task,Name=:Name,Efficiency=:Efficiency,Timeliness=:Timeliness,Quality=:Quality,Accuracy=:Accuracy,Remarks=:Remarks,Evalouter=:Evalouter WHERE id=:id");
