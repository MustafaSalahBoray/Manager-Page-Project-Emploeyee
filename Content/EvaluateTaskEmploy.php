		...
		<div class="card-body " >
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
                          ...
                          $admn_task=$db->prepare("SELECT * FROM emploeyeetask WHERE id=:id");
                            $admn_task->bindparam("id",$_GET['Evaluate']);
                          $admn_task->execute();
                          foreach ($admn_task as $key ) {
                           $user=explode(",", $key['Emploey_Name']);
                           
                          for ($i=0; $i <count($user) ; $i++) { 
                          	// 	   $admn=$db->prepare("SELECT * FROM emploeyeetask WHERE 	Emploey_Name=:id");
					...
					<tr>
						...
						<td class="text-center">
							...
							<div class="dropdown-menu">
								 <?php 
								       if ($key['Stuts'] =='Done'){
								           echo '<a class="dropdown-item view_project" href="http://localhost/Manager%20Page%20Project%20Emploeyee/Content/Evaluate%20Employee.php?Evaluate='.$key['id'].'&&user='.$user[$i].'" data-id="">Evaluate</a>';}
